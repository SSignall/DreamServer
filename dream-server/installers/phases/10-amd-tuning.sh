...
if [[ "$GPU_BACKEND" == "amd" ]] && $DRY_RUN; then
    ...
    # Dynamically calculate GTT size based on total RAM — use ~90% for GPU, leave rest for OS/Docker
    # 90% is optimal for LLM inference per halo-ai-core and r/LocalLLaMA Strix Halo benchmarks
    total_ram_mb=$(awk '/MemTotal/ {printf "%d", $2/1024}' /proc/meminfo 2>/dev/null || echo "0")
    if [[ "$total_ram_mb" -gt 0 ]]; then
        # 90% of total RAM for GTT, in MB (e.g. 128GB → ~115GB, leaves ~13GB for OS/Docker)
        gtt_size=$(( total_ram_mb * 90 / 100 ))
...
# GTT = 90% of RAM (~${gtt_size}MB), leaving ~$((total_ram_mb - gtt_size))MB for OS/Docker
...
GTT_EOF
        if sudo -n cp /tmp/dream-gtt-tuning.conf /etc/modprobe.d/amdgpu_llm_optimized.conf 2>/dev/null; then
            ...
            ai_ok "GTT memory tuning installed (gttsize=${gtt_size}MB of ${total_ram_mb}MB, 90%)"
    ...
    # amd_iommu=off gives ~6% memory bandwidth improvement; NPU (XDNA2) doesn't
    # work on Linux yet so there's no downside for LLM inference workloads.
    if [[ -f /etc/default/grub ]]; then
        ...
        if [[ -n "$current_cmdline" ]] && ! echo "$current_cmdline" | grep -q 'amd_iommu=off'; then
            ...
            if echo "$current_cmdline" | grep -q 'iommu=pt'; then
                if sudo -n sed -i 's/iommu=pt/amd_iommu=off/' /etc/default/grub 2>/dev/null; then
