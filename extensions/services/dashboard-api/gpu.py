...
def _find_all_amd_gpu_sysfs() -> list[str]:
    """Find all AMD GPU sysfs paths."""
    import glob
    gpus = []
    for card_dir in sorted(glob.glob("/sys/class/drm/card*/device")):
        vendor = _read_sysfs(f"{card_dir}/vendor")
        if vendor == "0x1002":
            gpus.append(card_dir)
    return gpus
...
def get_gpu_info_amd_single(device_path: str) -> Optional[GPUInfo]:
    """Get GPU metrics from amdgpu sysfs for a single device."""
    try:
        ...
        gpu_name = _read_sysfs(f"{device_path}/product_name") or "AMD Radeon (Strix Halo)"
...
def get_gpu_info_amd() -> Optional[GPUInfo]:
    """Get GPU metrics from amdgpu sysfs (first GPU only for backward compatibility)."""
    base = _find_amd_gpu_sysfs()
    if not base:
        return None
    return get_gpu_info_amd_single(base)
...
def get_gpu_info() -> Optional[GPUInfo]:
    """Get primary GPU metrics. Tries AMD sysfs first (if GPU_BACKEND=amd), then NVIDIA."""
    gpus = get_all_gpus()
    if not gpus:
        return None

    # Return first GPU (primary) for backward compatibility
    return gpus[0]
